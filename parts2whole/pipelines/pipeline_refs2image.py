# Adapted from https://github.com/huggingface/diffusers/blob/v0.25.0/src/diffusers/pipelines/stable_diffusion/pipeline_stable_diffusion.py
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from parts2whole.pipelines.pipeline_image_pose2image import ImagePose2ImagePipeline

        >>> pipe = ImagePose2ImagePipeline.from_pretrained("huanngzh/animate-anyone-diffusers")
        >>> pipe = pipe.to("cuda")

        >>> ref_image = load_image("https://huggingface.co/datasets/huanngzh/testing-images/resolve/main/animate-anyone/ironman.png")
        >>> pose_image = load_image("https://huggingface.co/datasets/huanngzh/testing-images/resolve/main/animate-anyone/pose_0.jpg")
        >>> image = pipe(ref_image, pose_image).images[0]
        ```
"""
...
@dataclass
class Refs2ImagePipelineOutput(BaseOutput):
    """
    Output class for Refs-to-Image pipeline.
...
class Refs2ImagePipeline(
    ...
    Args:
        ...
        image_encoder ([`CLIPVisionModelWithProjection`]):
            Frozen image-encoder ([clip-vit-large-patch32](https://huggingface.co/openai/clip-vit-large-patch32)).
        reference_unet ([`ReferenceUNet2DConditionModel`]):
            A `ReferenceUNet2DConditionModel` to encode the reference image.
        unet ([`UNet2DConditionModel`]):
            A `UNet2DConditionModel` to denoise the encoded image latents.
        pose_guider ([`PoseGuider`]):
            A lightweight control network to guide the image generation process using pose condition.
        scheduler ([`SchedulerMixin`]):
    ...
    def __init__(
        ...
        self.register_modules(
            ...
            scheduler=scheduler,
            feature_extractor=feature_extractor or CLIPImageProcessor(),
    ...
    def clip_encode_image(self, image, device, num_images_per_prompt):
        dtype = next(self.image_encoder.parameters()).dtype
        ...
        image = image.to(device=device, dtype=dtype)
        image_embeds = self.image_encoder(image).image_embeds.unsqueeze(1)
        image_embeds = image_embeds.repeat_interleave(num_images_per_prompt, dim=0)
        uncond_image_embeds = torch.zeros_like(image_embeds)
    ...
    def _get_bg_color(
        self, bg_color: Union[str, float, np.ndarray], return_tensor: bool
    ):
        if bg_color == "white":
        ...
        if return_tensor:
            bg_color = torch.from_numpy(bg_color).to(dtype=torch.float32)
    ...
    def vae_encode_image(self, image, height, width, device, num_images_per_prompt):
        if not isinstance(image, torch.Tensor):
            image = self.image_processor.preprocess(image, height, width).to(
        ...
        uncond_image_latents = image_latents.clone()

        return image_latents, uncond_image_latents

    def encode_pose(
        self, pose, height, width, device, num_images_per_prompt, do_normalize=False
    ):
        if not isinstance(pose, torch.Tensor):
            pose = self.image_processor.preprocess(pose, height, width)
            if not do_normalize:
                pose = (pose + 1) / 2.0

        pose = pose.to(device=device, dtype=self.pose_guider.dtype)
        pose_fea = self.pose_guider(pose).repeat_interleave(
            num_images_per_prompt, dim=0
        )
        uncond_pose_fea = pose_fea.clone()

        return pose_fea, uncond_pose_fea
    ...
    def __call__(
        self,
        appearance: Dict[str, PipelineImageInput],
        structure: Dict[str, PipelineImageInput],
        mask_dict: Dict[str, PipelineImageInput] = None,
        ...
        ### NEW ###
        reference_fusion_blocks: str = "full",
        ...
        **kwargs,
        ...
        Args:
            ref_image (`PipelineImageInput`):
                The reference image to guide image generation.
            pose_image (`PipelineImageInput`):
                The pose image to guide image generation.
            prompt (`str` or `List[str]`, *optional*):
        ...
        # Define reference control operations
        reference_control_writer = ReferenceAttentionControl(
            self.reference_unet,
            do_classifier_free_guidance=self.do_classifier_free_guidance,
            mode="write",
            batch_size=batch_size,
            fusion_blocks=reference_fusion_blocks,
        )
        ...
        for key, value in appearance.items():
            ...
            value, mask = self.preprocess_image(value, height, width, bg_color="white")

            # Load mask from mask image if provided
            if mask_dict is not None and key in mask_dict:
                mask = self.load_mask(mask_dict[key], height, width)

            value = value.unsqueeze(0).to(device=device, dtype=self.vae.dtype)
            mask = mask.unsqueeze(0).to(device=device, dtype=self.vae.dtype)

            ref_latents, uncond_ref_latents = self.vae_encode_image(
            ...
            self.reference_unet(
                ...
                ref_timesteps,
                text_embeds,
                cross_attention_kwargs={"hidden_states_mask": mask},
            )

        # Update the reference attention features to the bank
        reference_control_reader.update(reference_control_writer)

        # Encode and inject reference appearance images and text embeddings
        encoder_hidden_states = []

        prompt_embeds_d, neg_prompt_embeds_d = self.encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            self.do_classifier_free_guidance,
        ...
        for key, value in appearance.items():
            image_embeds, uncond_image_embeds = self.clip_encode_image(
                value, device, num_images_per_prompt
            )
            if self.do_classifier_free_guidance:
                image_embeds = torch.cat([uncond_image_embeds, image_embeds])

            # Append the image embeddings to the list for denoising unet
            image_encoder_hidden_states_dunet.append(image_embeds)

        # Padding image_encoder_hidden_states_dunet to length 6 # FIXME: hard-code
        ...
        # Encode reference structure images
        pose_fea, uncond_pose_fea = self.encode_pose(
            list(structure.values())[0],
            height,
            width,
            device,
            num_images_per_prompt,
        ...
        if self.do_classifier_free_guidance:
            pose_fea = torch.cat([uncond_pose_fea, pose_fea])
        ...
        # Clear reference attention controller
        ...
        return Refs2ImagePipelineOutput(images=image)
